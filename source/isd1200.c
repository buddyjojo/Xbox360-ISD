...
{
	uint8_t buf[1 + 1 + data_len];
	buf[0] = CMD_SPI_SND_DEC;
	buf[1] = 0x78;  //This thing, doccumented in the notes
...
// const size_t chunk_payload = 41514;
// const size_t chunk_total   = 41516;

// void isd1200_playspi()
// {
// 	const size_t chunk_payload = 4089;
// 	const size_t chunk_total   = 4091;
//
// 	size_t offset = 0;
//
...
// 		size_t payload_len = remaining > chunk_payload ? chunk_payload : remaining;
//
// 		uint8_t buf[chunk_total];
// 		buf[0] = CMD_SPI_SND_DEC;
// 		buf[1] = 0x78;
// 		memcpy(&buf[2], &data[offset], payload_len);
//
// 		nuvoton_spi_transfer(buf, 2 + payload_len);
//
// 		isd1200_stop();
//
// 		offset += payload_len;
...
{
	uint8_t buf[1 + 3 + 512] = {CMD_DIG_READ, 0x00, 0x00, 0x00};

	uint32_t offset = page * 512;

	*(uint32_t *)&buf[1] = offset << 8;

	nuvoton_spi_transfer(buf, sizeof(buf));

	memcpy(buffer, &buf[4], 512);
}
...
void isd1200_flash_write(uint32_t page, uint8_t *buffer)
{
	...
	*(uint32_t *)&buf[1] = offset << 8;
	...
	if (page == 0 && buf[4] != 0xCF) {
		printf("ISD_ERR: Protection byte mismatch\n");
		return;
	}
